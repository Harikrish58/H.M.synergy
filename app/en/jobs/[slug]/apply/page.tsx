...
export async function generateMetadata({
  ...
  if (!job) {
    return {
      title: "Job Application | H&M Synergy",
      robots: {
        index: false,
        follow: true,
      },
  ...
  return {
    title: `Apply for ${job.title_en} | H&M Synergy`,
    description: `Apply for the ${job.title_en} position in ${job.location_en}. Submit your candidate details and CV to H&M Synergy.`,
    robots: {
      index: false,
      follow: true,
    },
...
export default async function JobApplicationPage({
      ...
      <main className="min-h-[70vh] bg-white px-6 py-20 sm:px-10 lg:px-8">
        <div className="mx-auto max-w-[1280px]">
          ...
          <p className="mt-5 max-w-xl text-base leading-7 text-[#5B6878]">
            The position you are trying to apply for is no longer available or
            does not exist.
          </p>
  ...
  /*
   * The current application route is English.
   * The localized title/location/category are therefore taken
   * from the English Supabase columns.
   */
  const localizedJob = {
    slug: job.slug,
    code: job.code,
   ...
   * from lib/jobs.ts.
   *
   * We temporarily provide the same minimal shape here so
   * the existing form continues to work while the public
   * application API is migrated to Supabase.
   */
        ...
        <div className="mx-auto grid max-w-[1280px] gap-12 px-6 py-20 sm:px-10 lg:grid-cols-[0.72fr_1fr] lg:gap-20 lg:px-8 lg:py-28">
          <div>
            ...
            <p className="mt-6 max-w-md text-base leading-7 text-[#5B6878] sm:text-lg sm:leading-8">
              Complete the form and upload your CV. Your application will be
              sent directly to our recruitment team for this position.
            </p>
