...
export async function generateMetadata({
  ...
  if (!job) {
    return {
      title: "Подати заявку | H&M Synergy",
      robots: {
        index: false,
        follow: true,
      },
  ...
  return {
    title: `Подати заявку: ${job.title_ua} | H&M Synergy`,

    description:
      `Подайте заявку на вакансію ${job.title_ua} у ${job.location_ua}. ` +
      "Надішліть свої дані та CV до H&M Synergy.",
    ...
    alternates: {
      languages: {
        en: `/en/jobs/${job.slug}/apply`,
        pl: `/pl/jobs/${job.slug}/apply`,
        uk: `/ua/jobs/${job.slug}/apply`,
      },
    },
  };
}

export default async function UkrainianJobApplicationPage({
  params,
}: PageProps) {
  const { slug } = await params;
      ...
      <main className="min-h-[70vh] bg-white px-6 py-20 sm:px-10 lg:px-8">
        <div className="mx-auto max-w-[1280px]">
          ...
          <p className="mt-5 max-w-xl text-base leading-7 text-[#5B6878]">
            Вакансія, на яку ви намагаєтеся подати заявку,
            більше недоступна або не існує.
          </p>
  ...
  /*
   * The Ukrainian application route uses the Ukrainian
   ...
   * from lib/jobs.ts.
   *
   * We provide the same compatible shape here while the
   * actual vacancy remains stored in Supabase.
   */
        ...
        <div className="mx-auto grid max-w-[1280px] gap-12 px-6 py-20 sm:px-10 lg:grid-cols-[0.72fr_1fr] lg:gap-20 lg:px-8 lg:py-28">
          <div>
            ...
            <p className="mt-6 max-w-md text-base leading-7 text-[#5B6878] sm:text-lg sm:leading-8">
              Заповніть форму та додайте своє CV.
              Ваша заявка буде безпосередньо передана
              нашій рекрутинговій команді для цієї вакансії.
            </p>
