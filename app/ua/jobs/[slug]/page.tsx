...
export async function generateMetadata({
  ...
  if (!job) {
    return {
      title: "Вакансію не знайдено | H&M Synergy",
      robots: {
        index: false,
        follow: false,
      },
  ...
  return {
    title: `${job.title_ua} | ${job.location_ua} | H&M Synergy`,

    description:
      `Подайте заявку на вакансію ${job.title_ua} у ${job.location_ua}. ` +
      "Перегляньте деталі вакансії, вимоги, обов'язки, " +
      "заробітну плату та інформацію про подання заявки.",
...
export default async function UkrainianJobDetailPage({
  params,
}: Props) {
  const { slug } = await params;
